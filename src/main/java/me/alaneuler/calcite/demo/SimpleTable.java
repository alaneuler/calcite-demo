...
public class SimpleTable extends AbstractTable implements ScannableTable, ProjectableFilterableTable {
    ...
    private Enumerable<Object[]> getObjects(DataContext root) {
    ...
    private List<CsvFieldType> getCsvFieldType() {
        ...
        for (SqlTypeName sqlTypeName : fieldTypes) {
            switch (sqlTypeName) {
                case VARCHAR -> csvFieldTypes.add(CsvFieldType.STRING);
                case INTEGER -> csvFieldTypes.add(CsvFieldType.INT);
                case DECIMAL -> csvFieldTypes.add(CsvFieldType.DOUBLE);
                default -> throw new RuntimeException("Unsupported type " + sqlTypeName + " in csv");
            }
        }
