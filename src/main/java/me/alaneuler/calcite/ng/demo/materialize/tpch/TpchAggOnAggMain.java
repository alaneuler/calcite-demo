...
public class TpchAggOnAggMain extends TpchBaseMain {
  public static void main(String[] args) {
    ...
    printInfo("Query 26, aggregate on aggregate, case 2 mv-agg on agg, " + SUBSTITUTION.name(),
        result);
    ...
    result = routine(SUBSTITUTION, sql, mvSql);
    printInfo("Query 26, aggregate on aggregate, case 3 mv-second agg, " + SUBSTITUTION.name(),
        result);
    result = routine(SPJG, sql, mvSql);
    ...
    result = routine(SPJG, sql, FileUtils.getFileContent("tpch/mvs/26-2.sql"),
        FileUtils.getFileContent("tpch/mvs/26-3.sql"));
    printInfo("Query 26, aggregate on aggregate, case 4 mv-(first and second agg), " + SPJG.name(),
        result);
    ...
    result = routine(SUBSTITUTION, sql, mvSql);
    printInfo("Query 13, aggregate on aggregate, case 2 mv-agg on agg, " + SUBSTITUTION.name(),
        result);
    result = routine(SPJG, sql, mvSql);
    ...
    result = routine(SUBSTITUTION, sql, mvSql);
    printInfo("Query 13, aggregate on aggregate, case 3 mv-second agg, " + SUBSTITUTION.name(),
        result);
    result = routine(SPJG, sql, mvSql);
